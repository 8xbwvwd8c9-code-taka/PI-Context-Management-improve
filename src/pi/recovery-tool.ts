...
import {
	...
	type ToolResultPersistenceError,
 ...
 */
export function executePicmRecover(
	args: unknown,
	opts: ExecuteRecoveryOptions,
): RecoveryToolOutput {
	...
	if (parsed === null) {
		...
		if (!/^[a-z0-9_-]{8,128}$/.test(refStr)) {
			throw new ToolResultAccessError(
				`picm_recover: ref must be a cmv3://tool/<id> URI or a bare opaque id (got ${JSON.stringify(refStr)})`,
			);
		}
	}
	...
	try {
		bytes = opts.store.toolResults.readRange(
			canonicalRef,
			opts.projectId,
			range.start,
			range.end,
		...
		if (err instanceof ToolResultAccessError) throw err;
		throw new ToolResultAccessError(
			`picm_recover: read failed (${(err as Error).message})`,
		);
	}
...
function normalizeInput(args: unknown): RecoveryToolInput {
	if (args === null || typeof args !== "object") {
		throw new ToolResultAccessError("picm_recover: args must be an object");
	}
	const a = args as Record<string, unknown>;
	const out: RecoveryToolInput = {
		ref: typeof a["ref"] === "string" ? (a["ref"] as string) : "",
	};
	if (a["start"] !== undefined) {
		const s = a["start"];
		if (typeof s !== "number" || !Number.isInteger(s) || s < 0) {
			throw new ToolResultAccessError(
				`picm_recover: start must be a non-negative integer (got ${JSON.stringify(s)})`,
			);
		}
		(out as { start: number }).start = s;
	}
	if (a["end"] !== undefined) {
		const e = a["end"];
		if (typeof e !== "number" || !Number.isInteger(e) || e < 0) {
			throw new ToolResultAccessError(
				`picm_recover: end must be a non-negative integer (got ${JSON.stringify(e)})`,
			);
		}
		(out as { end: number }).end = e;
	}
	if (a["full"] !== undefined) {
		const f = a["full"];
		if (typeof f !== "boolean") {
			throw new ToolResultAccessError(
				`picm_recover: full must be a boolean (got ${JSON.stringify(f)})`,
			);
		}
		(out as { full: boolean }).full = f;
	}
	if (a["max_bytes"] !== undefined) {
		const m = a["max_bytes"];
		if (typeof m !== "number" || !Number.isInteger(m) || m <= 0) {
			throw new ToolResultAccessError(
				`picm_recover: max_bytes must be a positive integer (got ${JSON.stringify(m)})`,
			);
		}
		// Hard ceiling — the recovery tool cannot be made to
		// return more than 1 MiB even if the caller asks.
		(out as { max_bytes: number }).max_bytes = Math.min(
			m,
			MAX_RECOVERY_RANGE_BYTES,
		);
	}
...
function computeRange(
	input: RecoveryToolInput,
	originalBytes: number,
): { start: number; end: number; bounded_by_cap: boolean } {
	if (input.full === true) {
	...
	if (start > originalBytes) {
		throw new RangeError(
			`picm_recover: start (${start}) escapes artifact length (${originalBytes})`,
		);
	}
	if (end > originalBytes) {
		throw new RangeError(
			`picm_recover: end (${end}) escapes artifact length (${originalBytes})`,
		);
	}
	if (end < start) {
		throw new RangeError(
			`picm_recover: end (${end}) must be >= start (${start})`,
		);
	}
	return { start, end, bounded_by_cap: false };
}

function renderBytesAsText(
	bytes: Uint8Array,
	originalBytes: number,
	range: { start: number; end: number; bounded_by_cap: boolean },
	full: boolean,
): string {
...
function lossyPrintable(bytes: Uint8Array): string {
	let out = "";
...
// Suppress unused-import lint: we want to expose the
// `ToolResultPersistenceError` type for the extension to import
// alongside `executePicmRecover`.
