...
export type ReconcileAction =
	| { kind: "complete"; newSessionId: string; rolloverId: string }
	| {
			kind: "fail";
 ...
 */
export function reconcileInterruptedRollovers(
	...
	try {
		...
		for (const rollover of executing) {
			...
			if (action.kind === "incomplete") {
				...
				const diag: ReconcileDiagnostic = {
					level: "warning",
					message:
						`P04 reconciliation: ambiguous candidates for rollover ${action.rolloverId} ` +
				...
				const diag: ReconcileDiagnostic = {
					level: "info",
					message:
						`P04 reconciliation: rollover ${action.rolloverId} marked FAILED ` +
				...
				const diag: ReconcileDiagnostic = {
					level: "info",
					message:
						`P04 reconciliation: rollover ${action.rolloverId} marked COMPLETE ` +
		...
		const diag: ReconcileDiagnostic = {
			level: "error",
			message:
				`P04 reconciliation: exception during scan: ${message}. ` +
...
/* -------------------------------------------------------------------- *
 * Per-rollover reconciliation                                           *
 * -------------------------------------------------------------------- */

interface ReconcileOneCtx {
	readonly projectId: string;
	readonly oldSessionId: string;
	readonly store: Cmv3Store;
	readonly jsonlScanner: JsonlParentScanner;
...
function reconcileOne(
	...
	let durableLookup: ChildLookupResult;
	try {
	...
	if (jsonlLookup.kind === "one") {
		try {
			ctx.store.rollovers.transition({
				...
				failureDetail:
					"ghost child exists but S04 durable session linkage never completed",
				now: ctx.now(),
			});
			return {
				kind: "fail",
	...
	try {
		ctx.store.rollovers.transition({
			...
			failureCode: "new_session_failed",
			failureDetail:
				"no child evidence at startup; rollover interrupted before side effect",
			now: ctx.now(),
		});
		return {
			kind: "fail",
...
export function findDurableChild(
	...
	if (matches.length === 1) return { kind: "one", childId: matches[0] };
...
function listExecutingRollovers(
	store: Cmv3Store,
	projectId: string,
): RolloverRequest[] {
...
function classifyChildren(children: readonly string[]): ChildLookupResult {
...
export function normalizeSessionId(raw: string): string {
	...
	if (stripped.length === 0 || stripped === "current") return "";
	return stripped;
}
 ...
 */
export function previousSessionRefFor(raw: string): string | null {
 ...
 * extension.ts.
 */
export function consoleDiagnosticSink(): DiagnosticSink {
	return {
		emit(diagnostic: ReconcileDiagnostic): void {
