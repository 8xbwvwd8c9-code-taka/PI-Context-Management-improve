...
export const PRESSURE_STATES: readonly PressureState[] = Object.freeze([
	"NORMAL",
	"TARGET_EXCEEDED",
	"SWEEP",
	"CHECKPOINT",
...
export function classifyPressure(
	...
	if (!Number.isFinite(usage.tokens) || usage.tokens < 0) {
		throw new Error(
			`classifyPressure: tokens must be a finite number >= 0 (got ${usage.tokens})`,
		);
	}
 ...
 */
export function headroomToEmergency(
	usage: ContextUsage,
	profile: ContextProfile,
): number {
