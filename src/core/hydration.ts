 ...
 */
export function buildHydrationPayload(handoff: MinimalHandoff): HydrationPayload {
 ...
 * turn markers.
 */
export function looksLikeTranscriptPayload(text: string): boolean {
	const t = text.toLowerCase();
	if (t.includes("<user>") && t.includes("</user>")) return true;
	if (t.includes("<assistant>") && t.includes("</assistant>")) return true;
	if (t.includes("system:") && t.includes("user:")) return true;
	return false;
}
