...
export class History {
    stores;
    constructor(_layout, stores) {
        this.stores = stores;
    }
    list(q) {
        const kinds = q.kinds ?? ["checkpoint", "handoff", "session", "tool"];
        const out = [];
        ...
        if (kinds.includes("session")) {
            ...
            for (const s of ss) {
                if (q.workPackage)
                    continue; // sessions do not carry work_package
                if (q.status && s.status !== q.status)
                    continue;
                if (q.since && s.started_at < q.since)
                    continue;
                if (q.until && s.started_at > q.until)
                    continue;
                if (q.toolName)
                    continue;
                if (q.sessionId && s.id !== q.sessionId)
                    continue;
                out.push({
                    kind: "session",
                    id: s.id,
    ...
    find(ref, projectId) {
        ...
        if (ref.startsWith("cmv3://tool/")) {
            if (!this.stores.toolResults)
                return null;
            const summaries = this.stores.toolResults.list(projectId);
            const match = summaries.find((s) => s.ref === ref);
            if (!match)
                return null;
            return {
