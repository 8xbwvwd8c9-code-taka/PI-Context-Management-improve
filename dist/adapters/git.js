 ...
 * best-effort.
 */
export function discoverGitProject(root) {
    ...
    const headSha = detachedSha ?? (branch === null ? null : readRefOrNull(join(gitDir, "refs", "heads", branch)));
    ...
    const dirty = existsSync(indexPath) ? null : null;
    const info = {
        ...base,
        repository: origin,
        branch,
 ...
 */
export class GitProjectAdapter {
    id = "git";
    root;
    constructor(root) {
        this.root = root;
...
function readOriginUrl(gitConfigPath) {
    let raw;
    try {
