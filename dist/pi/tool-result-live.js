 ...
 */
import { buildFailureActiveView, DEFAULT_ACTIVE_VIEW_POLICY, } from "../core/tool-result.js";
/* -------------------------------------------------------------------- *
 * Constants                                                             *
 * -------------------------------------------------------------------- */
/**
 ...
 */
export function virtualizeToolResult(input, opts) {
    if (opts.mode === "legacy") {
        return {};
    }
    ...
    const result = opts.store.toolResults.write(bytes, {
 ...
 */
export function failOpenForPersistenceError(input, err, policy = DEFAULT_ACTIVE_VIEW_POLICY) {
    const bytes = serializeContentToBytes(input.content);
 ...
 * Pure helpers                                                          *
 * -------------------------------------------------------------------- */
function textByteLength(content) {
    let n = 0;
...
function serializeContentToBytes(content) {
    ...
    for (const c of content) {
        if (c.type === "text") {
            ...
            parts.push(`[image:${c.mimeType ?? "unknown"}:${Buffer.byteLength(c.data, "base64")}b]`);
        }
    }
    return new TextEncoder().encode(parts.join("\n"));
}
function inferResultKind(content) {
    if (content.length === 0)
        return "empty";
    let hasImage = false;
    let hasText = false;
 ...
 */
function renderActiveViewAsText(input, view, originalBytes) {
    ...
    if (view.truncated) {
        head.push(`[active view bounded to ${view.active_excerpt_bytes} bytes; full result recoverable via ref]`);
    }
    else {
...
//# sourceMappingURL=tool-result-live.js.map